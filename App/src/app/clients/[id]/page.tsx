...
type Visit = {
  ...
  medicalFocusQuestionnaire?: any | null;
  sandstormQuestionnaire?: any | null;
  radiationQuestionnaire?: any | null;
};
          ...
          const visitsWithAttachments = await Promise.all(
            visitData.map(async (visit: Visit) => {
              const attachRes = await fetch(
                `/api/attachments?visitId=${visit.id}`,
              );
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-20 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-clean border border-gray-100 p-8 md:p-12">
        ...
        <div className="mt-14 pt-12 border-t border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-8">
          <HistoryItem
            label="Drug History (Dh)"
            name="drugHistory"
            value={client?.drugHistory}
            editValue={formData.drugHistory}
            isEditing={isEditing}
            onChange={setFormData}
          />
          <HistoryItem
            ...
            name="familyHistory"
            value={client?.familyHistory}
            editValue={formData.familyHistory}
            isEditing={isEditing}
            onChange={setFormData}
          />
          <HistoryItem
            label="Social History (Sh)"
            name="socialHistory"
            value={client?.socialHistory}
            editValue={formData.socialHistory}
            isEditing={isEditing}
            onChange={setFormData}
          />
        </div>
          ...
          <div className="grid gap-8">
            {visits.map((visit) => (
              ...
              >
                ...
                <QuestionnaireHistory visit={visit} />
...
function QuestionnaireHistory({ visit }: { visit: Visit }) {
  const blocks: Array<{ title: string; value: any }> = [
    {
      title: "Medical Focus Questionnaire",
      value: visit.medicalFocusQuestionnaire,
    },
    { title: "Radiation Questionnaire", value: visit.radiationQuestionnaire },
    { title: "Sandstorm Questionnaire", value: visit.sandstormQuestionnaire },
  ];

  const nonEmpty = blocks.filter((b) => b.value);
    ...
    <div className="mt-10 pt-8 border-t border-gray-50 space-y-4">
      ...
      <div className="space-y-3">
        {nonEmpty.map((b) => (
          ...
          >
            <summary className="cursor-pointer font-black text-gray-800 select-none">
              {b.title}
            </summary>

            <pre className="mt-3 text-xs lg:text-sm bg-white border border-gray-100 rounded-xl p-4 overflow-auto whitespace-pre-wrap break-words">
