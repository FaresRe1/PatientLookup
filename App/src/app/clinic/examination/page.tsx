...
function ExamCard({ encounter }: { encounter: EncounterRow }) {
  const { client } = encounter;
  const isActive = encounter.status === "in_examination";

  return (
    <Link
      href={`/clinic/examination/${encounter.id}`}
      className={`flex items-center gap-4 p-4 bg-white rounded-2xl border transition-all hover:shadow-md hover:-translate-y-0.5 group ${
        isActive ? "border-indigo-200 bg-indigo-50/30" : "border-gray-100 hover:border-brand-orange/30"
      }`}
    >
      {client.profileImage ? (
        <img
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1.5 flex-wrap">
          ...
          {(encounter.bloodPressure || encounter.heartRate || encounter.temperature) && (
...
export default function ExaminationQueuePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (encRes.ok) {
      ...
      setEncounters(
        all.filter((e) => e.status === "triage_completed" || e.status === "in_examination"),
      );
    }
    setLoading(false);
  }, []);
  ...
  const waiting = encounters.filter((e) => e.status === "triage_completed");
  const inProgress = encounters.filter((e) => e.status === "in_examination");
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-black text-gray-900">Examination Queue</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-gray-100">
          ...
          <p className="font-black text-gray-500 text-lg">No patients awaiting examination</p>
          <p className="text-sm text-gray-400 font-medium mt-1">
            Patients will appear here once triage is complete.
          </p>
        </div>
      ) : (
          ...
          {inProgress.length > 0 && (
            <section className="space-y-3">
              <h3 className="text-xs font-black text-indigo-600 uppercase tracking-[0.15em] flex items-center gap-2">
                ...
                In Examination ({inProgress.length})
              </h3>
              {inProgress.map((e) => <ExamCard key={e.id} encounter={e} />)}
            </section>
          )}
          {waiting.length > 0 && (
            <section className="space-y-3">
              <h3 className="text-xs font-black text-violet-600 uppercase tracking-[0.15em] flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-violet-500" />
                Awaiting Examination ({waiting.length})
              </h3>
              {waiting.map((e) => <ExamCard key={e.id} encounter={e} />)}
            </section>
          )}
        </div>
      )}
